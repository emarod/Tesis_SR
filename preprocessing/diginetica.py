...
REDUCTION_FACTOR = .3 # Keep 25% of the data
...
def process_times(session_times_original):
  output = []
  min_time = float("inf")
  max_time = float("-inf")
  ...
  for session in session_times_original:
    ...
    for idx in range(len(session)):
        norm_time = math.log(session[idx])
        min_time = min(min_time, norm_time)
        max_time = max(max_time, norm_time)
        norm_sess.append(norm_time)
    output.append(norm_sess)
  return output, min_time, max_time

# def process_times(session_times_original, mu_g=1):
#   """
...
#     if len(session) > 1:
#       for idx in range(1, len(session)):
#         edge = (math.log(session[idx-1] + 1), math.log(session[idx] + 1))
#         min_time = min(min_time, edge[0], edge[1])
#         max_time = max(max_time, edge[0], edge[1])
#         incoming_e.append(edge)
      
#       for idx in range(len(session) - 1):
#         edge = (math.log(session[idx] + 1), math.log(session[idx+1] + 1))
#         min_time = min(min_time, edge[0], edge[1])
#         max_time = max(max_time, edge[0], edge[1])
#         outgoing_e.append(edge)
        
#     output_incoming.append(incoming_e)
#     output_outgoing.append(outgoing_e)
        
#   return output_incoming, output_outgoing, min_time, max_time


# global_mean = calculate_global_mean_dwell_time(tr_times)
# tr_times, t_min, t_max = process_times(tr_times)
# te_times, a, b = process_times(te_times)

# --- INICIO DE AJUSTE MIN-MAX ---
# print("Aplicando normalización Min-Max [0, 1] a los tiempos...")
# print(f"Rango original detectado -> Min: {t_min:.4f}, Max: {t_max:.4f}")

# 2. Función auxiliar para escalar las secuencias
def apply_min_max(time_sequences, t_min, t_max):
    norm_seqs = []
    # Evitar división por cero en el caso rarísimo de que max == min
    denominator = (t_max - t_min) if (t_max - t_min) > 0 else 1.0
    
    for seq in time_sequences:
        norm_seq = [((u - t_min) / denominator, (v - t_min) / denominator) for u,v in seq]
        norm_seqs.append(norm_seq)
    return norm_seqs

# 3. Sobrescribir las listas con los valores escalados
# tr_times_in = apply_min_max(tr_times_in, t_min, t_max)
# tr_times_out = apply_min_max(tr_times_out, t_min, t_max)
# te_times_in = apply_min_max(te_times_in, a, b)
# te_times_out = apply_min_max(te_times_out, a, b)

# print(f"Ejemplo de tiempo normalizado [0, 1]: {tr_times[0][:4]}")
# --- FIN DE AJUSTE MIN-MAX ---
